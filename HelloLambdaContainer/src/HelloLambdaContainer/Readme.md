...
WORKDIR /src
COPY ["HelloLambdaContainer.csproj", "HelloLambdaContainer/"]
RUN dotnet restore "HelloLambdaContainer/HelloLambdaContainer.csproj"

WORKDIR "/src/HelloLambdaContainer"
COPY . .
RUN dotnet build "HelloLambdaContainer.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HelloLambdaContainer.csproj" \
...
Execute unit tests
```
cd "HelloLambdaContainer/test/HelloLambdaContainer.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "HelloLambdaContainer/src/HelloLambdaContainer"
